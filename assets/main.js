...
function createProductCard(product) {
    ...
    card.innerHTML = `
        ...
        <button class="button button--primary" onclick="handleAddToCart(${product.id})">
            Добавить в корзину
        </button>
    `;
...
async function loadProducts() {
    try {
        ...
        const data = await api.getProducts(currentPage, sortSelect.value);
...
async function searchProducts(query) {
    ...
    try {
        ...
        const response = await fetch(`${CONFIG.API_URL}/products/search?${params}`);
